...
public:
  ...
  //retourne n lignes a partir de line (inclu) (n = 0 : fin du fichier):
  std::vector<std::string> readLine(unsigned const &line, unsigned n);
  //supprime n lignes a partir de line (inclu) (n = 0 : fin du fichier):
  void removeLine(unsigned const &line, unsigned const &n = 1);
  //remplace le texte déjà existant à partir de la ligne n (n et tout ce qui suit sont remplacés)
  //n = 0 pour écrire sans rien remplacer à la fin du fichier
  void write(std::string const &text, unsigned n = 1);
  void write(std::vector<std::string> const &text, unsigned const &n = 1);
  //insert le texte avant la ligne n
  void insert(std::string const &text, unsigned const &n);
